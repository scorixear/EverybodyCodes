...
def main():
    ...
    while (
        ...
        # print_grid(max_grid_x, max_grid_y, min_grid_x, min_grid_y, seen, bone, curr_pos)
        step_count += 1
    print(step_count)
...
def flood_fill(
    ...
    if (x, y) in seen:
        return
    if do_check and can_fill(
        seen, bone, x, y, max_grid_x, max_grid_y, min_grid_x, min_grid_y
    ):
        seen.add((x, y))
        flood_fill(
            seen, bone, x + 1, y, max_grid_x, max_grid_y, min_grid_x, min_grid_y, False
        )
        flood_fill(
            seen, bone, x - 1, y, max_grid_x, max_grid_y, min_grid_x, min_grid_y, False
        )
        flood_fill(
            seen, bone, x, y + 1, max_grid_x, max_grid_y, min_grid_x, min_grid_y, False
        )
        flood_fill(
            seen, bone, x, y - 1, max_grid_x, max_grid_y, min_grid_x, min_grid_y, False
        )
