...
class Wall:
    ...
    def is_in_passage(self, x, y):
        return self.x == x and y >= self.y and y < self.y + self.height

    def is_in_wall(self, x, y):
        return self.x == x and (y < self.y or y >= self.y + self.height)
...
def main():
    ...
    while queue:
        ...
        new_pos1 = (next_x, y + 1)
        new_pos2 = (next_x, y - 1)
        if not any(wall.is_in_wall(*new_pos1) for wall in walls):
            queue.append((new_pos1, flaps + 1))
        if new_pos2[1] >= 0 and not any(wall.is_in_wall(*new_pos2) for wall in walls):
